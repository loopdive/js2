...
const DEFAULT_RUNTIME_ENVIRONMENTS = ["node", "browser"];
...
function isMissingExportError(error) {
  const message = error?.message || String(error);
  return /\b(JS|Wasm) export\b.*\bmissing\b/.test(message);
}

export async function runBrowserRuntimeBenchmarks(options = {}) {
  setStatus("Loading runtime benchmark manifest...");
  const includeRuntimeEnvironments = new Set(options.includeRuntimeEnvironments ?? DEFAULT_RUNTIME_ENVIRONMENTS);
  const skipMissingExports = options.skipMissingExports ?? true;
  ...
  const entries = (manifest?.benchmarks ?? []).filter((entry) =>
    includeRuntimeEnvironments.has(entry?.runtimeEnvironment ?? "node"),
  );
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (!skipMissingExports || !isMissingExportError(error)) {
        throw error;
      }
      skipped.push(entry.name);
      console.warn(`[runtime-benchmark] skipped ${entry.name}: ${error?.message || error}`);
    }
  }

  setStatus(
    `Done. Measured ${results.length} browser runtime benchmark${results.length === 1 ? "" : "s"}${
