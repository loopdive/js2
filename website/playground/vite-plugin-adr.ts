...
// Vite dev middleware that renders website/docs/adr/*.md → HTML on the fly so the
...
//   reaches the browser.
// - The source on disk under `website/docs/adr/` is `.md` only — without this plugin,
 ...
 * render. Returns the source filename (under `website/docs/adr/`) or `null` if the
...
export function adrPlugin(): Plugin {
  ...
  const adrDir = join(projectRoot, "website", "docs", "adr");
