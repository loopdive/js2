...
export function test262Plugin(): Plugin {
  const test262Root = join(projectRoot, "tests", "test262");
  const testBase = join(test262Root, "test");
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        // Serve static files from project root (benchmarks/, tests/test262/)
        ...
        if (url.pathname.startsWith("/benchmarks/") || url.pathname.startsWith("/test262/")) {
          ...
          const sourcePath = url.pathname.startsWith("/test262/")
            ? join(test262Root, url.pathname.slice("/test262/".length))
            : join(projectRoot, url.pathname);
          const filePath = normalize(existsSync(publicPath) ? publicPath : sourcePath);
