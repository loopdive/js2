...
**Strategy B — Define a new Wasm-native language or incompatible JS dialect**. TypeScript syntax with
...
Excellent performance because the language is Wasm-shaped. But it is *not*
an ECMAScript compatibility path — existing JavaScript and npm packages
cannot be used without rewriting. Breaks the compatibility goal.
...
embedded runtime; no custom GC; no new language or permanent incompatible
subset/superset/dialect.
...
- **Strategy B** rejected because ECMAScript compatibility is a hard requirement.
