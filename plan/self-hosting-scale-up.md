...
| 2   | **parse/format** — `parse-number-native.ts` (1,924) + `number-format-native.ts` (1,535)                                           |           ~700 |                                                          **−2.8k** | **IN PROGRESS — #3305.** Slice 1 (`number_toString_radix`) landed net-negative. Slice 2's two-limb `parseFloat` carrier is bit-correct, but its first AnyString reader checkpoint is rejected: 4.03× median scan time and +19 production LOC. The amended implementation hoists one flat `$__str_data` descriptor into a ctx-bound private source signature, uses direct absolute raw loads, and promotes only private scan indexes to i32 if the raw/f64 prototype still misses slope parity. The exact i64 finalizer and #2654/#4234 pow10 kernel remain unchanged; no generic inference/resolver widening.                                                                                                                     | Precursor A LANDED (#3256). Hard gates now require direct-load WAT with no per-character cast/field reload, interleaved 1/8/20/60-digit slope ratios ≤1.10 (paired 95% upper ≤1.15), raw-bit equivalence, one effective flatten/no allocation, and an honestly net-negative one-helper diff. Failed raw/i32 or LOC stages remain HOLD; `__str_to_number` and `parseInt` do not silently amortize the slice. Medium risk.                                                                                                                       |
