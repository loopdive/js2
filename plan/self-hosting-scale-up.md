...
| 2   | **parse/format** — `parse-number-native.ts` (1,924) + `number-format-native.ts` (1,535)                                           |           ~700 |                                                          **−2.8k** | **IN PROGRESS — #3305.** Slice 1 (`number_toString_radix`) landed net-negative. The 2026-08-26 current-main audit corrected the old “mirror op-for-op” claim: decimal parsing's i64 mantissa is outside the ordinary TS dialect. Slice 2 moves only the `parseFloat` scanner to TS using two exact base-1e9 f64 limbs; one narrow fixed-ABI native finalizer reconstructs i64 and reuses the unchanged #2654/#4234 pow10 kernel. No general inference/global-resolver widening; `__str_to_number` and `parseInt` follow independently.                                                                                                                                                                                                     | Precursor A LANDED (#3256). Current plan keeps one effective flatten/no allocation, requires raw-bit main-vs-candidate A/B plus throughput evidence, and stops if the one-helper checkpoint is not production-net-negative. Medium risk.                                                                                                                                                                                                                                                                                                               |
