...
| **1979** | IR: mid-body `if (cond) stmt;` in a void function silently skips ALL subsequent statements when cond is true | 61 | ready | high |
| **1980** | IR: while/for with a numeric-truthiness condition emits invalid Wasm and bricks the entire module (no fallback, verifier silent) | 61 | ready | high |
| **1981** | IR: === null / !== null on class-typed values statically folded to false/true — null guards silently deleted | 61 | ready | high |
| **1982** | IR: lazy use-site emission reorders memory reads past writes — slot/class-field reads observe future mutations | 61 | ready | critical |
