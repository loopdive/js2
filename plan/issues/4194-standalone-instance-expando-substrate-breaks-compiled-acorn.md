...
loc-budget-allow:
  # New module: the whole substrate (carrier predicate, bag get/set, tombstone
  # resurrect, and the `__extern_set` declared-field write-through ladder). It
  # is a NEW file precisely so `object-runtime.ts` does not regrow — the
  # `vec-props.ts` / `carrier-bag-*.ts` pattern.
  - src/codegen/instance-props.ts
  # `object-runtime.ts` is already a tracked god-file, so even the four small
  # edits this needs (reserve call, `__extern_get` else-branch consult + one
  # scratch local, `__extern_set` miss-arm composition, bag-keys append in the
  # shared enumeration builder) trip the ceiling. Deliberately kept to call-site
  # wiring — every new body lives in `instance-props.ts`.
