...
## ⚠ What is NOT verified — this is the important section

- **No test262 measurement.** None. Not stale, *absent*.
- **No kill-switch attribution**, no regression control, no funnel.
- Nobody has confirmed the two new modules do what their names say.
...
1. Establish whether the Annex B cancel path and `named-this-call` actually fix
   #4023 / #4025 — measure, do not assume from the module names.
2. Run the funnel per stage with a force-refreshed baseline, kill-switch
   attribution, and every apparent regression re-run **solo**.
3. Either land it properly with numbers, or close this and let the pin
   (`refs/rescued/workspace-staged-20260802`) be the record.
