...
standalone baseline (`pass: 21184`, full corpus). Its normal update path only
ratchets upward; the exceptional #5317 policy reseed is recorded below.
...
- with `--update`, RAISES the committed mark when pass improved and does not
  lower it. An explicit reviewed commit can still change the policy file, as
  #5317 did during the 2026-08-30 recovery event.
...
## 2026-08-30 incident — authenticate the pre-existing 1,295-pass slide

The #2097 backstop blocked the protected IR merge queue after exposing a
standalone host-free decline that predates the queued IR branches. The first
response was a fix-forward recovery plan against the 33,876 mark. PR #5317
subsequently made a different explicit policy decision: it reseeded the mark
to the already-published 32,581 candidate measurement. That protected-main
decision supersedes the old-floor recovery requirement, but it does not erase
the historical transition or make unauthenticated aggregate counts acceptable
evidence.

### Superseding policy event — PR #5317

PR #5317 merged as `d6c97cd28bf05d…` on 2026-08-30. Its content commits
changed the committed high-water source to
`01fb67624e2f645b7e92dd9f8e47478e3face9ba`, with
`host_free_pass=32,581`, tolerance 50, and therefore an effective floor of
32,531. Current-main acceptance follows that committed policy. This issue and
its verifier do not change the reseeded file, restore the former floor by
documentation, or claim that 32,581 was recovered through compiler work.

The diagnostic checkpoint instead preserves an authenticated comparison of
the former 33,876 endpoint and the newly authoritative 32,581 endpoint. Phase A
below is implemented by the bounded verifier and its mutation suite. Phases B
and C remain a forensic playbook only; any renewed compiler-recovery program
must be filed and accepted as separate work against current main rather than
silently reinstating the superseded #2097 gate.
...
The `01fb676…` forced refresh first supplied an authenticated measurement;
publishing those bytes did not itself alter the high-water policy. PR #5317
later adopted that same measurement as the new mark. #5297 is the first
...
- Do not change PR #5295 or its semantic acceptance contract. #5308 is already
  merged; retain its historical measurement without asking the queue to
  synthesize it again.
- Do not modify the current #5317 high-water file or tolerance in this
  diagnostic checkpoint, switch the gate from `host_free_pass`, skip the
  required job, or relabel known historical outcomes as passes.
...
If attribution is resumed, file a separate bounded repair issue naming the
exact production files before assigning implementation to Terra. Coordinate
with the parallel Claude Program ABI/#3525 work and preserve every unrelated
worktree change.
...
### Optional forensic Phase B — locate every causal boundary
...
If separately authorized, the output is a signed causal ledger mapping every
one of the 1,764 endpoint regressions to a first causal commit or an explicit
still-unattributed HOLD. Do not begin a broad production repair while any large
path family remains unattributed. This ledger is not required to accept the
bounded Phase A diagnostic checkpoint.

### Optional forensic Phase C — classify before repairing
...
File one bounded repair issue per independent cause. Each slice must name exact
files, path set, expected pass recovery, mutation controls, and rollback
behavior before code edits begin.

### Acceptance matrix

The bounded diagnostic checkpoint is accepted only when all of the following
hold:

1. The immutable 33,876 and 32,581 endpoint bytes authenticate against the
   pinned baseline-store commits, trees, blobs, lengths, hashes, producer
   schema, report vectors, and exact 48,735-key census before parsing.
2. The canonical transition artifact reproduces 1,764 regressions, 469
   improvements, net −1,295, and its published semantic/transition digests;
   every fail-closed mutation and reorder control passes.
3. The checkpoint changes only this plan plus the verifier and focused test. It
   neither changes compiler behavior nor modifies the #5317 mark of 32,581,
   tolerance 50, or effective floor 32,531.
4. Documentation states that #5317 superseded the former 33,826 acceptance
   floor. The historical 33,876 endpoint remains evidence, not current policy,
   and no compiler recovery is claimed.
5. #5308 remains recorded as already merged. #5295 proceeds, if otherwise
   approved, through the normal protected queue against the current floor; no
   admin merge, direct merge, or queue bypass is permitted.
