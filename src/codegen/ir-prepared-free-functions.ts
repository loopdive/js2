...
export function prepareIrFreeFunctionBodies(input: {
  ...
  const routing = preparedIrBodyRouting(initialReport, input.claimsByUnitId, { deferUnsupported: true });
...
export function prepareIrClassMemberBodies(input: {
  ...
  const selection: IrSelection = {
    ...
    classMemberUnitIds: memberUnitIds,
    moduleInit: undefined,
  };
  const stagedTopLevelAccessorSetterUnitIds = stageSelectedTopLevelAccessorSetterAbis({
    ctx: input.ctx,
    identityPlan: input.identityPlan,
    unitIds: memberUnitIds,
    classShapesById: input.classShapesById,
  });
  const initialReport: IrIntegrationReport = withSelectedTopLevelAccessorUnitIds(
  ...
  const deferredBodies = new Set(deferredProjection.entries.map(({ legacyName }) => legacyName));
  const completedBodyUnitIds = new Set([...memberUnitIds].filter((unitId) => !routing.deferredUnitIds.has(unitId)));
  return {
    report,
    requestedSkipProjection,
    ...
    skipBodies: new Set(requestedSkipProjection.entries.map(({ legacyName }) => legacyName)),
    preserveBodies: new Set(preparedProjection.entries.map(({ legacyName }) => legacyName)),
    completedBodyUnitIds,
    skipBodyUnitIds: routing.irOwnedUnitIds,
    preserveBodyUnitIds: routing.preparedUnitIds,
  };
}
