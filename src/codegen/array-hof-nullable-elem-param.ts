 ...
 */
export function nullableElemParamOverrideFor(elemType: ValType | undefined): ValType | undefined {
  return elemType !== undefined && elemType.kind === "ref_null" ? elemType : undefined;
}

/**
 * Apply the override to a checker-resolved parameter type.
 *
 * Returns the override **only** when `resolved` is the exact non-null twin of
 * it (`ref X` against `ref_null X`). Every other pair — a different `typeIdx`,
 * an `externref`/scalar parameter, an already-nullable parameter, an absent
 ...
 */
export function applyNullableElemParamOverride(resolved: ValType, override: ValType | undefined): ValType {
  if (override === undefined || override.kind !== "ref_null") return resolved;
  if (resolved.kind !== "ref") return resolved;
  return resolved.typeIdx === override.typeIdx ? override : resolved;
}
