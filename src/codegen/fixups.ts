 ...
 */
function instrPopsPushes(instr: Instr, mod: WasmModule): { pops: number; pushes: number } | null {
...
function callTargetFuncType(instr: Instr, mod: WasmModule): FuncTypeDef | null {
 ...
 */
function locateCallArgProducers(instrs: Instr[], mod: WasmModule): Map<number, number[]> {
  ...
  for (let i = 0; i < instrs.length; i++) {
    ...
    if (op === "call" || op === "return_call") {
      ...
      if (n > producers.length) break; // underflow — cannot model
      out.set(i, producers.slice(producers.length - n));
    }
