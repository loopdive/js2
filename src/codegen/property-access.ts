...
export function taViewReceiverTypeIdx(
  ...
  if (localIdx !== undefined) {
    ...
    // (#5150) …or a MODULE GLOBAL with the same slot type. test262 declares its
    // bindings at top level (`var ta = new Uint8Array(buffer)`), so the
    // locals-only lookup missed every one of them and `ta.length` fell to the
    // checker-typed vec arm — which `ref.test`s a different struct and answers 0.
