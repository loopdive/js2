...
/**
 * (#5139) Pre-root the override slot for every `Array.prototype` iterator key
 * the source assigns, BEFORE any function body compiles.
 *
 * The read-drive gate is `arrayIteratorMaybeOverridden && override-captured`,
 * and "captured" used to become true only when the module-init assignment
 * itself compiled. A class method body compiles FIRST, so the gate read
 * `undefined` there and the parameter lane silently took the backing-store fast
 * path — the override was installed but never observed. Rooting the slot during
 * the existing S1 pre-scan makes the gate order-independent; the global stays
 * `ref.null.extern` until the assignment stores into it, and `arrayDstrNeedsIdentity`
 * still gates everything on the S1 brand, so a program without an override is
 * unaffected.
 *
 * Only the keys the source actually assigns are rooted: rooting `@@iterator`
 * for a program that overrides `values` would shadow the real slot in
 * `arrayIteratorOverrideGlobalIdx`'s `@@iterator ?? values` lookup.
 */
export function rootArrayProtoIteratorOverrideSlots(ctx: CodegenContext, sourceFile: ts.SourceFile): void {
  if (!ctx.arrayIteratorMaybeOverridden) return;
  const keys = new Set<string>();
  const walk = (node: ts.Node): void => {
    if (ts.isBinaryExpression(node) && node.operatorToken.kind === ts.SyntaxKind.EqualsToken) {
      const key = arrayProtoIteratorOverrideKeyFromTarget(node.left);
  ...
  for (const memberKey of keys) {
    if (inner.has(memberKey)) continue;
    const globalIdx = nextModuleGlobalIdx(ctx);
    ctx.mod.globals.push({
      name: `__array_proto_${memberKey === "@@iterator" ? "iterator" : memberKey}_override`,
      type: { kind: "externref" },
...
/**
 * (#5139) `delete Array.prototype[Symbol.iterator]` had no compiled landing
 * spot at all — the delete lowering dropped it, so a later array destructuring
 * kept using the backing store instead of raising the §7.4.2 GetIterator
 * TypeError ("method is not callable"). Model it as one `mut i32` module global
 * raised by the delete and read by the array-pattern lanes.
 *
 * Like the override slot, the global is rooted during the S1 pre-scan so a
 * method body compiled before `__module_init` still sees it.
 */
const arrayProtoIteratorDeletedGlobal = new WeakMap<object, number>();

/** True when `target` is `Array.prototype[Symbol.iterator]` / `Array.prototype.values`. */
export function isArrayProtoIteratorDeleteTarget(target: ts.Expression): boolean {
  return arrayProtoIteratorOverrideKeyFromTarget(target) !== undefined;
}

/** Root the "iterator was deleted" flag when the source contains such a `delete`. */
export function rootArrayProtoIteratorDeletedSlot(ctx: CodegenContext, sourceFile: ts.SourceFile): void {
  if (arrayProtoIteratorDeletedGlobal.has(ctx)) return;
  let found = false;
  const walk = (node: ts.Node): void => {
    if (found) return;
    if (ts.isDeleteExpression(node) && isArrayProtoIteratorDeleteTarget(node.expression)) {
      found = true;
      return;
    }
    ts.forEachChild(node, walk);
  ...
  arrayProtoIteratorDeletedGlobal.set(ctx, globalIdx);
}

/** The rooted flag global, or `undefined` when the source never deletes the key. */
export function arrayProtoIteratorDeletedGlobalIdx(ctx: CodegenContext): number | undefined {
  return arrayProtoIteratorDeletedGlobal.get(ctx);
}
