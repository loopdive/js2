...
/**
 * (#4639 C4) Delete the three RegExp prototype flag accessors without
 * seeding them into the companion object.
 *
 * ## The measured defect
 *
 * The RegExp flag accessors are configurable, but their `$NativeProto` CSV
 * is immutable to the ordinary delete path. The ES5 rows require
 * `delete RegExp.prototype.{global,ignoreCase,multiline}` to succeed AND make
 * `RegExp.prototype.hasOwnProperty('global')` answer `false` afterwards.
 * Measured on upstream/main (`--target standalone`, the three Test262 A9 rows):
 *
 * | step                                        | base     | spec     |
 * | ------------------------------------------- | -------- | -------- |
 ...
 * ## Scope
 *
 * This is deliberately a RegExp-only, three-key arm. The earlier generic
 * builtin-prototype delete arm was retired after its broad interception
 * regressed descriptor and method-routing rows; this bounded arm does not
 * touch methods, other builtins, or the accessor seeding/read path.
 *
...
const REGEXP_DELETE_KEYS = ["global", "ignoreCase", "multiline"] as const;

const DELETE_FN = "__regexp_proto_delete";

/**
 * Register `__regexp_proto_delete(obj externref, key externref) -> i32`: 1 when
 * `obj` is the RegExp builtin `$NativeProto` and one of the three accessor
 * tokens has been removed from its mutable member CSV, else 0.
 */
function registerRegExpProtoDelete(ctx: CodegenContext): number | undefined {
  ...
  const body: Instr[] = [
    ...
    { op: "if", blockType: { kind: "empty" }, then: returnZero },
    // Only the RegExp builtin owns this accessor set. Keeping the brand check
    // here prevents a same-named property on another native prototype from
    // entering this deletion arm.
    ...protoRef,
    { op: "struct.get", typeIdx: protoTypeIdx, fieldIdx: NP_BRAND },
    { op: "i32.const", value: BUILTIN_BRAND_TABLE.RegExp },
    { op: "i32.ne" },
    { op: "if", blockType: { kind: "empty" }, then: returnZero },
    ...
    // Bound the arm to the three ES5 accessor-delete rows. In particular,
    // `toString` and every data method remain on the untouched delete path.
    { op: "i32.const", value: 0 },
    { op: "local.set", index: L_ALLOWED },
    ...REGEXP_DELETE_KEYS.flatMap((member) => [
      { op: "local.get", index: L_FKEY } as Instr,
...
/**
 * Finalize splice: teach `__delete_property` about the three RegExp
 * prototype accessor members.
 *
 ...
 */
export function unshiftRegExpProtoDeleteArm(ctx: CodegenContext): void {
  const funcIdx = registerRegExpProtoDelete(ctx);
