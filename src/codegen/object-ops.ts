...
export function emitDefinePropertyFlagCheck(
  ...
  // (#1915) All four strings are materialized via stringConstantExternrefInstrs:
...
export function compileObjectDefineProperty(
  ...
  if (valueExpr && useStruct) {
    ...
    if (needsValueCompare) {
      ...
      // (#1915) Sentinel-safe string materialization (see flag-check above).
  ...
  if (allFieldNames.size > 0) {
    ...
    if (keyType) {
      ...
      if (eqFunc !== undefined && eqFunc >= 0) {
        ...
        fctx.body.push({ op: "i32.const", value: 0 });
        for (const fieldName of allFieldNames) {
          // (#1915) `>= 0` skips the nativeStrings -1 sentinel — this path is
