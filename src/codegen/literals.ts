  ...
  for (const prop of expr.properties) {
    ...
    if (
      ...
      for (const param of prop.parameters) {
        ...
        // Also handles #862: iterables passed to destructuring params get routed
        // through __extern_to_array via the externref path.
      ...
      for (let pi = 0; pi < prop.parameters.length; pi++) {
        ...
        // Also handles #862: untyped destructuring params may receive any iterable;
        // externref + __extern_to_array path handles all iterables uniformly.
        const hasBindingPattern = ts.isArrayBindingPattern(param.name) || ts.isObjectBindingPattern(param.name);
        if (hasBindingPattern && !param.type && !param.dotDotDotToken && wasmType.kind !== "externref") {
          wasmType = { kind: "externref" };
        }
