...
import type { ProgramAbiInventory } from "./abi-inventory.js";
import { irSourceGlobalBindingKey } from "../core/binding-key-primitives.js";
...
export interface ProgramAbiOrderKey {
  /** Dependency-first source order from {@link ProgramAbiInventory.sources}. */
 ...
 */
export class ProgramAbiMap<TInventory extends ProgramAbiInventory = ProgramAbiInventory> {
  ...
  private readonly sourceOrders = new Map<ProgramAbiInventory["sources"][number]["id"], number>();
  private readonly units = new Map<IrUnitId, ProgramAbiKnownUnitRecord>();
  private readonly classes = new Map<IrClassId, ProgramAbiInventory["classes"][number]>();
  ...
  private validateInventoryMembership(entry: ProgramAbiPlanEntry): void {
    ...
    if (entry.intent.kind === "global" && entry.intent.capability !== undefined) {
      ...
      const exactReferenceKey = irSourceGlobalBindingKey(entry.id, entry.intent.capability);
  ...
  private assertInventorySourceOrder(
    ...
    sourceId: ProgramAbiInventory["sources"][number]["id"],
  ): void {
