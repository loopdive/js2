...
function generateImportsHelper(adapterManifest: JavaScriptAdapterManifestV1): string {
  ...
  return [
    ...
    "//",
    "// `instantiateBytes`/`instantiateFromResponse` also return `exports`: the",
    "// module's exports already marshalled into plain JS values (struct graphs,",
    "// typed arrays, native strings). Reach for the raw `instance.exports` only",
    "// when you want the opaque WasmGC handles.",
    "",
    'import { buildCompiledAdapterImports, instantiateWasm, instantiateWasmStreaming, wrapExports } from "js2wasm";',
    "",
    ...
    "}",
    "",
    "// The compile-time equivalent of `wrapCompiledExports(result, instance)`:",
    "// the export metadata an API caller forwards by hand from the CompileResult",
    "// is already serialized in the manifest above, so a CLI consumer gets the",
    "// same marshalling without ever holding a CompileResult. Without this, a",
    "// post-CLI consumer instantiates fine and still only sees opaque WasmGC",
    "// handles — an exported function returning an object graph reads as empty.",
    "export function wrapInstance(instanceOrExports, options) {",
    "  return wrapExports(instanceOrExports, {",
    "    ...options,",
    "    signatures: adapterManifest.exportSignatures,",
    "    boundaryPolicies: adapterManifest.exportBoundaries,",
    "  });",
    "}",
    "",
    ...
    "  return { ...result, imports, exports: wrapInstance(result.instance, options?.wrap) };",
    "}",
    "",
