...
Direct AOT compilation from ECMAScript and TypeScript source to WebAssembly GC.
...
The compatibility target is ECMAScript itself, with TypeScript supported as a source-language layer rather than as a different language contract. Test262 is the current public measurement baseline, not the limit of the compatibility goal.

`js2wasm` is the core compiler product of **Loopdive GmbH**, released under **Apache License 2.0 with LLVM Exceptions**. The public repository contains the compiler source, tests, reports, and documentation needed to evaluate and contribute to the compiler itself.
...
- embedders that want ECMAScript semantics without shipping an interpreter
...
The current public benchmark and conformance work is aimed at proving that direct compilation can become a viable alternative to interpreter bundling for production infrastructure. The published Fibonacci numbers are standalone datapoints, not the scope of the project; current compiler work is also aimed at popular npm packages and real application code.

Many alternatives in adjacent spaces solve the problem by narrowing the language instead:

- supporting only a constrained subset of TypeScript or JavaScript semantics
- introducing a custom sub- or superset that compiles to Wasm more easily

`js2wasm` is aimed at a harder target: preserving mainstream ECMAScript semantics through direct compilation rather than changing the language model to fit the compiler.
...
- **63.1% Test262 compliance** as a public ECMAScript conformance baseline

See the [Playground](https://js2.loopdive.com/playground) and [Roadmap](./ROADMAP.md) for the current public surface.
...
- active work on popular npm package support beyond the public standalone microbenchmarks
...
3. custom JavaScript or TypeScript sub- or supersets, and new languages,
...
but every deployed module pays for that runtime. Custom sub- or superset and
new-language approaches can generate compact Wasm by changing the JavaScript
language contract.

`js2wasm` is aimed at a different target: **mainstream ECMAScript semantics
without shipping a JavaScript engine or interpreter inside the deployed
artifact**, with TypeScript source support layered on top rather than used as a
...
For a more detailed category-level comparison, see the [FAQ](./website/docs/faq.md).
...
The foundational design choices behind `js2wasm` — why WasmGC instead of linear memory, why AOT instead of an embedded engine, how TypeScript annotations are treated, how closures are lowered — are documented as Architecture Decision Records in [`website/docs/adr/`](./website/docs/adr/README.md). Each record states the context, the decision, and the consequences in 200–600 words. Start with [ADR-002 (architectural approach)](./website/docs/adr/0002-architectural-approach.md) and [ADR-001 (hybrid compilation strategy)](./website/docs/adr/0001-hybrid-compilation-strategy.md); the rest are sub-decisions within that frame.

## Further Reading

- [Playground](https://js2.loopdive.com/playground)
- [Roadmap](./ROADMAP.md)
- [Architecture Decisions](./website/docs/adr/README.md)
