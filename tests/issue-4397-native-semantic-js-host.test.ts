...
describe("#4397 native semantics in a JavaScript environment", () => {
  ...
  it("keeps Wasm-owned objects live and identity-stable at the JS boundary", async () => {
    ...
    // The assertions below require the KEY to be gone, not merely undefined:
    // biome's autofix (`open.label = undefined`) keeps it and breaks Object.keys.
    // biome-ignore lint/performance/noDelete: asserts real property removal
    expect(delete open.label).toBe(true);
