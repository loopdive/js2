...
//
// #1296 Tier 1 — dogfood: compile the analytics fixture with js2wasm itself,
...
describe("#1296 Tier 1 — analytics fixture compiles and aggregates correctly", () => {
  beforeAll(async () => {
    const src = readFileSync(ANALYTICS_PATH, "utf-8");
    const r = compile(src, { fileName: "tests/fixtures/dashboard-analytics.ts" });
    if (!r.success) {
      throw new Error(
        `analytics fixture failed to compile: ${r.errors.map((e) => `L${e.line}: ${e.message}`).join("; ")}`,
      );
    }
