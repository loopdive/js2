 ...
 * fixture would skip.
 *
 * 1. **OFF is byte-identical.** The flag defaults off and every wired call site
 *    falls through to the legacy emission.
...
describe("#4157 provably-non-null TypeError elision", () => {
  it("OFF is the default and ON changes the emitted binary", async () => {
    const [off, unset, on] = await Promise.all([build("0"), build(undefined), build("1")]);
    expect(Buffer.from(unset)).toEqual(Buffer.from(off));
  ...
  it("answers match JavaScript under both flag states", async () => {
    // (11 + 31) * 2 + 1
    const expected = 85;
  ...
  it("still throws on a genuinely null receiver under both flag states", async () => {
    for (const flag of ["0", "1"]) {
      const exports = await instantiate(await build(flag));
      expect(() => exports.nullReceiver!()).toThrow();
    }
  });
});
