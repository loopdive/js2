...
describe("#1058 nominal token projection", () => {
    ...
    const result = await compileMulti(
      {
        "./src/compiler/types.ts": `
          ...
          // The brands are never actually given values. At runtime they have zero cost.
          export interface TypeNode extends Node { _typeNodeBrand: any; }
          export interface TypeNode extends Node { readonly value: number; }
        `,
        "./entry.ts": `
          import type { TypeNode } from "./src/compiler/types.js";
  ...
  it("keeps a constant-computed syntax brand physical when source observes it", async () => {
    const result = await compileMulti(
      {
        "./src/compiler/types.ts": `
        ...
        `,
        "./entry.ts": `
          import type { Expression } from "./src/compiler/types.js";
          const key = "_" + "expressionBrand";
          const value = { kind: 1, [key]: 42 } as Expression;
          export function run(): number {
            return (value as any)[key] * 10 + Object.keys(value).length;
          }
        `,
      },
      "./entry.ts",
