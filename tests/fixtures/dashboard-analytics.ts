...
//
// #1296 Tier 1 — pure-compute aggregation kernels for report data.
//
// These functions are compiled to Wasm by js2wasm itself and used by the
// GitHub Pages site to aggregate the test262 trend and status counts. The kernels intentionally take raw
// `number[]` parameters (not full run/issue objects) so the module compiles
// without touching `JSON.parse`, `fetch`, or any DOM API. The report boot
// script or test harness is responsible for unpacking the underlying
// JSON shape into parallel primitive arrays before invoking these kernels.
//
// This file is the dogfood proof that the pure-compute path through js2wasm
// works end-to-end on real report math.
...
/**
 * Report data filters runs by minimum total (20000 for
...
function fixtureStatuses(): number[] {
  // 12 sample statuses across representative lanes:
