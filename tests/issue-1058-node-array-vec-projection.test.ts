...
describe("#1058 NodeArray vec projection", () => {
  it.each(["gc", "standalone"] as const)(
    "preserves expando metadata while widening the element carrier in %s",
    async (target) => {
      const result = await compile(
        ...
        export function erasedMetadata(): number {
          const value: unknown = makeDerived();
          return fingerprintFromExtern(value);
        }

        export function assignedMetadata(): number {
          const holder: { nodes: NodeArray<Node> } = { nodes: [] as any };
          const dynamic: any = holder;
          dynamic.nodes = makeDerived();
          return fingerprint(holder.nodes);
        }
        ...
        export function sharedMetadata(): number {
          const original = makeDerived() as MutableNodeArray<DerivedNode>;
          const projected = widen(original) as MutableNodeArray<Node>;
          projected.pos = 31;
          original.end = 41;
          if (original.pos !== 31 || projected.end !== 41) return -1;
          delete (projected as any).hasTrailingComma;
          if (Object.prototype.hasOwnProperty.call(original, "hasTrailingComma")) return -2;
          Object.setPrototypeOf(projected, { marker: 23 });
          if (Object.getPrototypeOf(original).marker !== 23) return -3;
          Object.setPrototypeOf(original, { marker: 29 });
          if (Object.getPrototypeOf(projected).marker !== 29) return -4;
          return 1;
        }
      ...
      const exports = (target === "standalone"
        ? instance.exports
        : wrapExports(instance, { signatures: result.exportSignatures })) as unknown as {
        test(): number;
        callbackTest(): number;
        sourceMetadata(): number;
        sharedMetadata(): number;
        erasedMetadata(): number;
        assignedMetadata(): number;
      };
      expect(exports.sourceMetadata()).toBe(171591);
      expect(exports.test()).toBe(171591);
      expect(exports.callbackTest()).toBe(171591);
      expect(exports.erasedMetadata()).toBe(171591);
      expect(exports.assignedMetadata()).toBe(171591);
      expect(exports.sharedMetadata()).toBe(1);
      ...
      expect(
        rawExports.__call_fn_method_1(undefined, rawExports.getFingerprintWidened(), rawExports.makeDerived()),
      ).toBe(171591);
      ...
      expect(rawExports.__call_fn_method_1(undefined, rawExports.getFingerprintFromExtern(), mirroredNodes)).toBe(
        171591,
      );
    },
  );
