...
const ENTRY_PATH = "examples/benchmarks.ts";
const HELPERS_PATH = "examples/benchmarks/helpers.ts";

function readPlaygroundModule(relativePath: string): string {
  return readFileSync(resolve(__dirname, "../website/playground", relativePath), "utf8");
}

function compilePlaygroundBenchmarks() {
  return compileMulti(
    {
      [ENTRY_PATH]: readPlaygroundModule(ENTRY_PATH),
      [HELPERS_PATH]: readPlaygroundModule(HELPERS_PATH),
    },
    ENTRY_PATH,
    {},
  );
...
describe("full playground benchmark source", () => {
  it("compiles without errors", () => {
    const result = compilePlaygroundBenchmarks();
    if (!result.success) {
