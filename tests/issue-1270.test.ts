...
// at runtime (see #1200 and the wasm-opt coverage notes for the
...
const WASM_DIS = join(process.cwd(), "node_modules", ".bin", "wasm-dis");
...
function watFor(src: string): string {
  ...
  return execSync(`${WASM_DIS} ${wasmPath}`, { encoding: "utf-8" });
}
