...
describe("Regression fixes for PR #68 regressions (#1022)", () => {
  ...
  it("map timeout: map with length=Infinity falls back to __proto_method_call (Fix A)", async () => {
    const r = await runTest262(test262TestPath("built-ins/Array/prototype/map/15.4.4.19-3-14.js"));
    expect(r).toBe("PASS");
  });

  it("filter getter: filter falls back to __proto_method_call (Fix A)", async () => {
    const r = await runTest262(test262TestPath("built-ins/Array/prototype/filter/15.4.4.20-9-c-i-30.js"));
    expect(r).toBe("PASS");
  });

  it("filter call-with-boolean: filter falls back (Fix A)", async () => {
    const r = await runTest262(test262TestPath("built-ins/Array/prototype/filter/call-with-boolean.js"));
    expect(r).toBe("PASS");
  });

  it("map call-with-boolean: map falls back (Fix A)", async () => {
    const r = await runTest262(test262TestPath("built-ins/Array/prototype/map/call-with-boolean.js"));
    expect(r).toBe("PASS");
  });

  it("reduce call-with-boolean: reduce falls back (Fix A)", async () => {
    const r = await runTest262(test262TestPath("built-ins/Array/prototype/reduce/call-with-boolean.js"));
    expect(r).toBe("PASS");
  });

  it("reduce length-getter throws: reduce falls back (Fix A)", async () => {
    const r = await runTest262(test262TestPath("built-ins/Array/prototype/reduce/15.4.4.21-5-12.js"));
    expect(r).toBe("PASS");
  });
  ...
  it("every call-with-boolean: void arrow () => {} on boolean receiver (Fix B+D)", async () => {
    const r = await runTest262(test262TestPath("built-ins/Array/prototype/every/call-with-boolean.js"));
    expect(r).toBe("PASS");
  });

  it("some call-with-boolean: void arrow () => {} on boolean receiver (Fix B+D)", async () => {
    const r = await runTest262(test262TestPath("built-ins/Array/prototype/some/call-with-boolean.js"));
    expect(r).toBe("PASS");
  });

  it("findIndex call-with-boolean: void arrow () => {} on boolean receiver (Fix B+D)", async () => {
    const r = await runTest262(test262TestPath("built-ins/Array/prototype/findIndex/call-with-boolean.js"));
    expect(r).toBe("PASS");
  });
  ...
  it("find return-abrupt-from-this: null receiver throws TypeError (Fix C)", async () => {
    const r = await runTest262(test262TestPath("built-ins/Array/prototype/find/return-abrupt-from-this.js"));
    expect(r).toBe("PASS");
  });

  it("findIndex return-abrupt-from-this: null receiver throws TypeError (Fix C)", async () => {
    const r = await runTest262(test262TestPath("built-ins/Array/prototype/findIndex/return-abrupt-from-this.js"));
    expect(r).toBe("PASS");
  });

  // Previously passing tests should still pass
  it("every.call with 3-param callback (regression guard)", async () => {
    const r = await runTest262(test262TestPath("built-ins/Array/prototype/every/15.4.4.16-3-24.js"));
    expect(r).toBe("PASS");
  });

  it("every.call with 3-param named function (regression guard)", async () => {
    const r = await runTest262(test262TestPath("built-ins/Array/prototype/every/15.4.4.16-3-1.js"));
    expect(r).toBe("PASS");
  });
});
