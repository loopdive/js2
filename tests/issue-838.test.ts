 ...
 * vec in BOTH the host/gc and standalone/WASI lanes — an f64 cannot represent a
 * 64-bit BigInt. BigInt is already a first-class `{ kind: "i64", bigint: true }`
 * value in the compiler, so `array.get`/`array.set` on the i64 backing array
 * need no packing/unpacking, and ToBigInt64/ToBigUint64 (both reduce mod 2^64)
 * come for free from i64 wraparound.
 *
...
describe("#838 BigInt64Array / BigUint64Array typed arrays", () => {
  ...
  it("BigInt64Array from an array-literal initializer", async () => {
    const exports = await compileToWasm(`
      export function test(): bigint {
    ...
    `);
    expect(exports.test()).toBe(45n);
  });

  it("BigUint64Array from an array-literal initializer", async () => {
    const exports = await compileToWasm(`
      export function test(): bigint {
