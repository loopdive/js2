...
describe("#3520 production class-support callable Program ABI planning", () => {
  it("uses the synthesized identity context for compatibility-mode class resolution", () => {
    ...
    expect(ctx.irUnitFuncMap.has(implicitConstructor!.id)).toBe(true);
  });

  it("publishes an empty implicit constructor as its exact source unit, never fabricated support", () => {
    const source = `
      class Empty {
    ...
    const fabricated = irSupportFuncRef(emptyClassId, "class-constructor", "Empty_new");
    expect(entries.some((entry) => entry.id === fabricated.binding.bindingId)).toBe(false);
  });
  ...
  it("publishes a relocated parent init and preserves the IR super path after DCE", async () => {
    ...
    const supportRef = irSupportFuncRef(parentClassId, "class-constructor-init", "__cm$A_init");
