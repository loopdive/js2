...
//
// Why: `benchmarks/results/test262-current.jsonl` is the local regression
// analysis baseline. If the committed file gets corrupted (mass-rewritten by
// a malformed merge, desynced by a workflow bug), the bucket analysis silently
// produces wrong answers and the merge gate becomes unreliable.
//
...
async function main(): Promise<void> {
  ...
  // Cheaper alternative: build path from baseline entry directly.
  const TEST262_ROOT = resolve(ROOT, "tests", "test262");
