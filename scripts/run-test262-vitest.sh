...
if [ "$USE_WORKTREE" = "1" ]; then
  rm -rf "$WT_DIR/node_modules" "$WT_DIR/tests/test262"
  ln -s "$MAIN_DIR/node_modules" "$WT_DIR/node_modules"
  mkdir -p "$WT_DIR/tests"
  ln -s "$MAIN_DIR/tests/test262" "$WT_DIR/tests/test262"
fi

# Verify symlinks
if [ ! -d "$WT_DIR/tests/test262/test" ]; then
  echo "ERROR: test262 symlink failed"
  exit 1
fi
echo "test262 symlink OK ($(ls "$WT_DIR/tests/test262/test/" | wc -l) dirs)"

# Share the disk cache
mkdir -p "$MAIN_DIR/tests/.test262-cache"
if [ "$USE_WORKTREE" = "1" ]; then
  ln -sf "$MAIN_DIR/tests/.test262-cache" "$WT_DIR/tests/.test262-cache"
fi
...
if [ -n "$CHUNKS" ]; then
  ...
  if [ ${#forwarded_args[@]} -gt 0 ]; then
    VITEST_RUN_TIMEOUT_MS="${VITEST_RUN_TIMEOUT_MS:-90m}" node scripts/run-vitest.mjs run tests/test262-chunk*.test.ts \
      --reporter=verbose \
  ...
  if [ ${#forwarded_args[@]} -gt 0 ]; then
    VITEST_RUN_TIMEOUT_MS="${VITEST_RUN_TIMEOUT_MS:-90m}" node scripts/run-vitest.mjs run tests/test262-vitest.test.ts \
      --reporter=verbose \
      "${forwarded_args[@]}" 2>&1 | tee /tmp/test262-vitest-run.log || true
  else
    VITEST_RUN_TIMEOUT_MS="${VITEST_RUN_TIMEOUT_MS:-90m}" node scripts/run-vitest.mjs run tests/test262-vitest.test.ts \
