 ...
 *   /test262/...         → tests/test262/ directory (source files)
...
const TEST262_ROOT = join(ROOT, "tests", "test262", "test");
...
const server = createServer((req, res) => {
  ...
  const filePath = url.startsWith("/test262/")
    ? join(ROOT, "tests", "test262", url.slice("/test262/".length))
    : join(ROOT, url);
