 ...
 *   website/public/benchmarks/results/size-benchmarks.json  (copy for Vite dev server)
 *
 * Usage:
...
const WEBSITE_ROOT = path.resolve(ROOT, "website");
const HELPERS_PATH = path.resolve(WEBSITE_ROOT, "playground", "examples", "benchmarks", "helpers.ts");
const RESULTS_PATH = path.resolve(ROOT, "benchmarks", "results", "size-benchmarks.json");
const PUBLIC_PATH = path.resolve(WEBSITE_ROOT, "public", "benchmarks", "results", "size-benchmarks.json");
const LOADTIME_RESULTS_PATH = path.resolve(ROOT, "benchmarks", "results", "loadtime-benchmarks.json");
const LOADTIME_PUBLIC_PATH = path.resolve(WEBSITE_ROOT, "public", "benchmarks", "results", "loadtime-benchmarks.json");
const LOADTIME_RESULTS_DIR = path.resolve(ROOT, "benchmarks", "results", "loadtime");
const LOADTIME_PUBLIC_DIR = path.resolve(WEBSITE_ROOT, "public", "benchmarks", "results", "loadtime");
const BINARYEN_BUNDLE_PATH = path.resolve(ROOT, "node_modules", "binaryen", "index.js");
const LANDING_PATH = path.resolve(WEBSITE_ROOT, "index.html");
...
// ---------------------------------------------------------------------------
// Benchmark files (website/playground/examples/benchmarks/)
// ---------------------------------------------------------------------------
...
function escapeHtmlForPre(code: string): string {
  return code.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
}

function replaceGeneratedSnippet(html: string, key: string, code: string): string {
  const pattern = new RegExp(`(<pre\\b(?=[^>]*data-generated-example="${key}")[^>]*>)[\\s\\S]*?(</pre\\s*>)`);
  if (!pattern.test(html)) {
    throw new Error(`Missing landing page generated snippet marker: ${key}`);
  }
  return html.replace(pattern, (_match, open: string, close: string) => {
    return `${open}\n${escapeHtmlForPre(code.trimEnd())}${close}`;
  });
}

function updateLandingHowItWorksSnippets(snippets: Record<string, string>): void {
  let html = fs.readFileSync(LANDING_PATH, "utf8");
  for (const [key, code] of Object.entries(snippets)) {
    html = replaceGeneratedSnippet(html, key, code);
  }
...
async function measureSizes(name: string, label: string, jsSrc: string, tsSrc: string): Promise<MeasuredSize | null> {
...
const fibMeasured = await measureSizes("fib", "fibonacci", HOW_FIB_JS, HOW_FIB_TS);
const fibEntry = fibMeasured?.entry ?? null;
if (fibEntry) {
  console.log(
...
const domMeasured = await measureSizes("dom", "DOM append", HOW_DOM_JS, HOW_DOM_TS);
const domEntry = domMeasured?.entry ?? null;
if (domEntry) {
  console.log(
...
if (!fibMeasured || !domMeasured) {
  throw new Error("Cannot update landing page snippets because a how-it-works example failed to compile.");
}

updateLandingHowItWorksSnippets({
  "how-fib-js": HOW_FIB_JS,
  "how-fib-wat": fibMeasured.wat,
  "how-dom-js": HOW_DOM_JS,
  "how-dom-wat": domMeasured.wat,
});
console.log("  landing snippets updated");
