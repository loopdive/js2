...
// test262 is nested under tests/ in the public repo, but local automation can
// run from nested worktrees. Walk up through likely workspace roots.
function findTest262Root(base: string): string {
  let current = base;
  for (let i = 0; i < 6; i++) {
    const candidates = [join(current, "tests", "test262"), join(current, "public", "tests", "test262")];
    for (const candidate of candidates) {
      if (existsSync(join(candidate, "test"))) return candidate;
    }
    current = join(current, "..");
  }
  return join(base, "tests", "test262"); // fall back; script will error with a clear path
}
