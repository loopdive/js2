...
const WEBSITE_PUBLIC = join(WEBSITE_ROOT, "public");
...
const PUBLIC_BENCH = join(WEBSITE_PUBLIC, "benchmarks", "results");
...
const TEST262_REPO_ROOT = join(ROOT, "tests", "test262");
const PLAYGROUND_EXAMPLES_DIR = join(WEBSITE_ROOT, "playground", "examples");
...
// Start from the Vite multi-page build, which includes the landing page
...
// Overwrite Vite-built report pages with the latest website/public versions (which include
// web components like <t262-donut> that Vite doesn't process).
const PUBLIC_REPORT = join(WEBSITE_PUBLIC, "benchmarks", "results", "report.html");
const PUBLIC_REPORT_SHORT = join(WEBSITE_PUBLIC, "benchmarks", "report.html");
...
// spec-compliance report data fetched by benchmarks/spec-compliance.html.
copyDirectoryIfExists(join(ROOT, "reports", "spec-compliance"), join(PAGES_DIST, "reports", "spec-compliance"));
...
// website/public/benchmarks/results/ for any files curated there.
...
copyFileIfExists(join(WEBSITE_ROOT, "frame-nav-sync.js"), join(PAGES_DIST, "frame-nav-sync.js"));
...
// on-origin /docs/adr/*.html instead of broken raw .md URLs.
