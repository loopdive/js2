...
**Compile mainstream ECMAScript and TypeScript source to WebAssembly — no interpreter, no runtime, smallest practical output.**

js2wasm is an ahead-of-time compiler that translates ECMAScript directly to WebAssembly using the GC proposal. Unlike approaches that embed a JS interpreter inside Wasm, js2wasm produces native Wasm instructions with no bundled runtime, allocator, or standard library. Unlike approaches that simplify compilation by changing the language contract, js2wasm targets ECMAScript compatibility over time rather than a custom sub- or superset. Output modules range from hundreds of bytes to a few kilobytes — orders of magnitude smaller than interpreter-based alternatives.

The goal: enable JavaScript and TypeScript developers to target WebAssembly as a first-class compilation target, producing modules that run on **any** Wasm host — browsers, WASI runtimes (Wasmtime, Wasmer, wazero), edge platforms, and embedded systems — without requiring a JavaScript engine at the destination. Test262 is the current public measurement baseline for ECMAScript conformance, not the limit of the compatibility goal; current work is also pushing support for popular npm packages and real application code.
...
- **27,213 / 43,160** Test262 tests passing (**63.1%** of the current public ECMAScript conformance baseline)
...
### Near-Term — Harden the 60%+ Conformance Baseline
