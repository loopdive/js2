...
- **Model split (2026-08-07): OPUS implements; FABLE specs the really hard tasks first** (architect pass writing the Implementation Plan before an Opus implementer takes it; honors issue-frontmatter `model: fable` + `fable_role: spec`) (`feasibility: hard` / `reasoning_effort: max` / core-dispatch changes / documented prior regressions): spawn with `model: "fable"` or inherit from a Fable main loop; do not default hard work to Opus. Honors issue-frontmatter `model: fable` — [hard-tasks-to-fable](feedback_hard_tasks_to_fable.md)
