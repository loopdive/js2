...
- [prettier-on-ignored-path-checks-zero-files](reference_prettier_check_on_ignored_path_checks_zero_files.md) — `.tmp/` is prettierignored; `--check` there passes having matched NOTHING. `--ignore-path /dev/null` for out-of-home checks
